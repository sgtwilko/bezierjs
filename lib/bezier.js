...
(function() {
  ...
  var utils = {
    ...
    abstractor: function() {
      ...
      return {
        addCoordinate: function(x,y,z) {
          coordinates.x.push(x);
          coordinates.x.push(y);
          coordinates.x.push(z||0);
        },
        abstract: function() {
          // for each dimension run through the point "curves
          // and try to find a Bezier fitting with reasonable
          // precision. This might require multiple curves as
          // there is no guarantee on the shape of the input.
        }
      };
    }
  };
  ...
  var Bezier = function(coords) {
    ...
    this._t1 = 0;
    this._t2 = 1;
    // precompute derivative points now, as a one-time action
    // rather than every time the derivative function(s) are called.
    this.dpoints = [];
