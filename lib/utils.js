module.exports = (function() {
  ...
  var BezierUtils = {
    ...
    contains: function(shape, p2) {
      var bbox = shape.bbox,
          edges = [shape.startcap, shape.forward, shape.endcap, shape.back],
          origin = { x: bbox.x.min - bbox.x.size, y: bbox.y.min - bbox.y.size },
          line = {p1: origin, p2: p2 },
          crossings = 0;
      edges.forEach(function(edge) {
        if (edge.lineintersects(line).length > 0) {
          crossings++;
        }
      });
    ...
    makeshape: function(forward, back) {
      ...
      shape.contains = function(p) {
        return self.contains(shape, p);
      };
      return shape;
    },
